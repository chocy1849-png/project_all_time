...
## DEC-025 — M7 Settings startup composition

Status: ACCEPTED

- `VNSettingsRuntimeBootstrap` owns the scene-session repository, service, five category runtime owners (including input rebinding), and panel initialization. It belongs on the existing `VNConvenienceRuntime` with explicit sibling validation for `VNConvenienceController` and `VNConvenienceInputRouter`; it never creates those components implicitly.
- Its four Inspector dependencies are the authoritative DialogueRunner, intended Settings Panel, project VNAudioMixer, and project VNInputActions. Plain runtime owners are not serialized. There is no singleton, service locator, polling loop, or persistent cross-scene object.
- Production startup creates `new VNSettingsRepository()`, constructs the service, and calls `Load()` once. `VNSettingsService.Current` remains the sole settings state authority; schema-v1 and M5 storage remain unchanged.
- Runtime application is synchronous in `Start`: Input overrides → Text / Auto → Gameplay → Audio → Display → Settings Panel initialization. `DefaultExecutionOrder(-1)` places this Start before Yarn's default-order Start and after scene Awake initialization. Installed Yarn 3.2.7 creates the LinePresenter typewriter in Awake; DialogueRunner.Start additionally awaits `YarnTask.Yield()` before its automatic start. No Yarn source or normal `M2_UI_START` node is changed.
