...
## M7-03 display runtime

- `VNResolutionOption` is an immutable width/height value whose equality and hash semantics deliberately exclude refresh rate. `VNDisplaySettingsController` exposes Windowed options and narrow apply/change methods without UI, scene, or repository access.
- The production display seam reads `Screen.resolutions`, `Display.main.systemWidth`, and `Display.main.systemHeight`, and requests resolution changes only through the explicit `Screen.SetResolution(width, height, FullScreenMode)` overload. Tests inject a fake seam rather than changing the Editor Game View.
- Windowed options discard non-positive sizes, deduplicate width × height, sort width then height ascending, and fall back to 1920×1080 when the runtime list is empty. Stale saved Windowed values resolve exact, then project default, then minimum squared distance to 1920×1080 with area/width/height tie-breaks.
- `windowedWidth`/`windowedHeight` remain the last valid Windowed choice during FullScreenWindow. User changes persist a `VNSettingsService.Current` snapshot before requesting a display change; `TryApplyCurrentSettings` applies an effective value without requiring a write, including while settings are write-protected. Unity applies a successful request after the current frame; M7-03 does not synchronously infer post-request Screen dimensions.
- Settings UI, bootstrap composition, confirmation/revert handling, monitor and refresh-rate selection, external OS display-change reconciliation, and non-display settings application remain deferred.

## M7-04 text and Auto speed runtime
...
## M7-05 audio settings runtime

- `IVNAudioMixerRuntime` is a narrow `GetFloat`/`SetFloat` seam. Production delegates to `AudioMixer`; EditMode tests use a fake and never mutate `VNAudioMixer.mixer` or global mixer state.
- `VNAudioSettingsController` owns exactly `MasterVolumeDb`, `BgmVolumeDb`, `SfxVolumeDb`, and `VoiceVolumeDb`. It converts normalized values to `Clamp(20 * Log10(value), -80, 0)`, with zero (and below) at -80 dB and one (and above) at 0 dB. Master/category values are independently applied—the Mixer graph supplies composition.
...
## M7-10 Settings production composition

- This integration supersedes the earlier M7 phase notes that defer startup composition. `VNSettingsRuntimeBootstrap` owns one `VNSettingsRepository`, one loaded `VNSettingsService`, Display/TextAuto/Audio/Gameplay controllers, and `VNInputRebindService` for the Scene session.
- Place it on the existing `VNConvenienceRuntime`; it validates both M6 sibling components explicitly. The only serialized fields are `dialogueRunner`, `settingsPanel`, `audioMixer`, and `inputActions`. The user attaches/wires this component in Unity; Codex does not modify Scene/Mixer YAML.
- `Start` at execution order -1 performs one Load, controller construction, then independent Input → Text/Auto → Gameplay → Audio → Display application, followed by `VNSettingsPanel.Initialize`. All scene Awake initialization completes first. Installed Yarn 3.2.7 initializes the ByLetter typewriter in Awake and its default-order automatic Start yields once before starting dialogue. The explicit project order therefore applies persisted text before normal dialogue without changing Yarn or `M2_UI_START`.
- Application delegates use existing controller seams and collect category diagnostics, including thrown failures. A recoverable category error does not suppress initialization of a correctly wired UI. Missing serialized/sibling references or invalid panel wiring abort initialization. Read-only diagnostic properties expose service, initialization, last diagnostic, and the future screen-shake gate; the UI still reads only the service snapshot.
- The bootstrap does not save effective defaults, bypass future-schema protection, apply Mixer settings in Awake/OnEnable, poll in Update, or persist across Scenes. Destruction safely disposes rebinding even after partial initialization.
- Enabled-action startup was verified against Input System 1.20.0 official JSON loading with the existing Router: enablement is retained and no startup shortcut callback fires. Production M6 input ownership and the rebinding implementation are unchanged.
- `VNMainM7SettingsWiringAuditTests` opens the Scene without saving and validates actual category/control references, persistent UnityEvents, slider contracts, modal state, CanvasScaler, EventSystem, input references, missing scripts, Mixer groups, and exact exposed parameters. Its `M7BootstrapSceneGate` test additionally requires the user-wired bootstrap exactly once on the existing M6 object.
